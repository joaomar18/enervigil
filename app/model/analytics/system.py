...
class DiskMetrics:
    """
    Container for disk performance metrics.

    This class encapsulates the key performance indicators related to disk
    usage and I/O activity. It is designed to be used as part of a larger
    system monitoring framework, providing a structured way to represent
    disk-related data.

    Attributes:
        usage (int): Amount of disk space currently in use, in bytes.
        total (int): Total available disk space, in bytes.
        read_bytes (int): Total bytes read from disk since boot.
        write_bytes (int): Total bytes written to disk since boot.
        usage_valid (bool): Indicates if the disk usage values are valid and can be trusted.
        io_valid (bool): Indicates if the disk I/O values are valid and can be trusted.
    """
...
class RealTimeSystemData:
    ...
    Attributes:
        ...
        disk_read: Total bytes read from disk since boot.
        disk_write: Total bytes written to disk since boot.
