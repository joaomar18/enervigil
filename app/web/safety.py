...
class HTTPSafety:
    ...
    USER_CONFIG_PATH = str(f"{APP_DATA_PATH}/user.json")
    ...
    def check_authorization_token(self, request: Request) -> Tuple[str, str, str]:
        ...
        if stored_token.user != username or stored_token.keep_session_until < date.get_current_utc_datetime():
            raise api_exception.TokenInRequestInvalid(api_exception.Errors.AUTH.INVALID_TOKEN)

        return (username, token, str(config["jwt_secret"]))

    def is_session_active(self, request: Request) -> bool:
        """
        Performs a lightweight authorization check for an existing session.

        Extracts the session token from the request and verifies that it corresponds
        to an active, non-expired server-side session. This method relies solely on
        in-memory state and does not perform JWT signature verification, payload
        decoding, or configuration access.

        Intended for fast, repeated authorization checks in long-lived contexts
        such as Server-Sent Events (SSE), where full authentication has already
        been performed at connection time.

        Args:
            request (Request): Incoming HTTP request containing the session token.

        Returns:
            bool: True if the token is present, recognized by the server, and the
            associated session is still active; False otherwise.
        """
        ...
        if stored_token.keep_session_until < date.get_current_utc_datetime():
            return False
