...
import media.image.device_img as device_img
...
async def add_device(
    ...
    try:
        ...
        if device_image:
            image_result = await asyncio.get_running_loop().run_in_executor(
                device_img.thread_executor, DeviceImageStorage.save_image, device_id, device_image, True
            )
            if not image_result:
        ...
        if device_id:
            await asyncio.get_running_loop().run_in_executor(
                device_img.thread_executor, DeviceImageStorage.delete_image, device_id
            )
        raise
...
async def edit_device(
    ...
    try:
        ...
        await conn.commit()
        await asyncio.get_running_loop().run_in_executor(device_img.thread_executor, DeviceImageStorage.flush_bin)

    except Exception:
        await conn.rollback()
        await asyncio.get_running_loop().run_in_executor(
            device_img.thread_executor, DeviceImageStorage.rollback_image, device_id
        )
        raise
...
async def get_all_devices(
    ...
    for device in device_manager.devices:
        ...
        current_status["alarm"] = any(
            [node for node in device.meter_nodes.nodes.values() if node.config.enabled and node.processor.in_alarm()]
        )
        current_status["warning"] = any(
            [node for node in device.meter_nodes.nodes.values() if node.config.enabled and node.processor.in_warning()]
        )
...
async def get_device_with_image(
    ...
    device_obj["image"] = await asyncio.get_running_loop().run_in_executor(
        device_img.thread_executor, DeviceImageStorage.get_image, device.id
    )
...
async def get_device_extended_info_with_image(
    ...
    device_info = await device.get_extended_info(database.get_device_history)
    device_info["image"] = await asyncio.get_running_loop().run_in_executor(
        device_img.thread_executor, DeviceImageStorage.get_image, device.id
    )
    return JSONResponse(content=device_info)
...
async def get_device_identification_with_image(
    ...
    device_identification["id"] = device.id
    device_identification["name"] = device.name
    device_identification["image"] = await asyncio.get_running_loop().run_in_executor(
        device_img.thread_executor, DeviceImageStorage.get_image, device.id
    )
    return JSONResponse(content=device_identification)
...
async def get_all_devices_with_image(
    ...
    for device in device_manager.devices:
        ...
        image_tasks.append(
            asyncio.get_running_loop().run_in_executor(device_img.thread_executor, DeviceImageStorage.get_image, device.id)
        )
        all_status.append(current_status)
...
async def get_default_image(request: Request, safety: HTTPSafety = Depends(services.get_safety)) -> JSONResponse:
    ...
    image = await asyncio.get_running_loop().run_in_executor(
        device_img.thread_executor, DeviceImageStorage.get_default_image
    )
