...
class APIMethodConfig:
    ...
    increment_exceptions: List[Type[api_exception.APIException]] = field(
        default_factory=list
    )  # Exception types that increment failed requests


def auth_endpoint(config: APIMethodConfig):
    """
    Decorator for defining authenticated and rate-limited API endpoints.

    Wraps an API endpoint function with centralized request safety handling,
    including client blocking, optional JWT-based authentication, failed
    request tracking, and standardized error response formatting.

    Behavior is configured via `APIMethodConfig`, allowing per-endpoint
    control over authentication requirements, rate limiting, and which
    exceptions contribute to failed request counts.
    """

    def decorator(func: EndpointFunc) -> Callable:
        """
        Wraps the endpoint function with authentication, rate limiting,
        and unified error handling logic.
        """

        @wraps(func)
        async def wrapper(request: Request, safety: HTTPSafety, **kwargs) -> JSONResponse:
            """
            Executes an API endpoint with centralized authentication, rate limiting,
            and error handling.

            Applies client blocking checks, optional JWT-based authentication, and
            failed-request tracking before invoking the wrapped endpoint function.
            Normalizes application-level and unexpected errors into structured JSON
            responses and updates request safety state accordingly.
            """
            ...
            try:
                ...
                logger.warning(
                    f"Failed {web_util.get_api_url(request)} API from IP: {web_util.get_ip_address(request)} due to error: {str(e.message)}"
                )
...
class AuthConfigs:
    ...
    LOGIN = APIMethodConfig(
        requires_auth=False, increment_exceptions=[api_exception.InvalidCredentials, api_exception.UserConfigurationNotFound]
    )
    AUTO_LOGIN = APIMethodConfig(requires_auth=False, enable_rate_limiting=False)
    LOGOUT = APIMethodConfig(
    ...
    CHANGE_PASSWORD = APIMethodConfig(
        increment_exceptions=[api_exception.InvalidCredentials, api_exception.UserConfigurationNotFound]
    )
