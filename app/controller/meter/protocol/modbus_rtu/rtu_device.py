...
class ModbusRTUEnergyMeter(EnergyMeter):
    ...
    Args:
        ...
        publish_data(Callable[[EnergyMeter], Awaitable[None]] | None):Optional callback triggered on device data publication.
    Attributes:
        ...
        force_nodes_disconnection (bool): Flag to force node disconnection state.
    Class attributes:
    ...
    def __init__(
        ...
        self.get_value_map: Dict[
            ModbusRTUNodeType, Callable[[ModbusRTUNodeOptions, ModbusPDU, int, int], float | int | bool]
        ] = {
    ...
    async def manage_connection(self):
        ...
        while self.run_connection_task:
            ...
            try:
                ...
                ):  # This loop will run forever because the driver connects to the virtual serial port, not the device itself
                    await asyncio.sleep(3)
    ...
    async def batch_read_nodes(
        ...
        if function not in self.modbus_function_map:
            raise ModbusException(
                f"Unknown modbus function {function} while trying to read batch group: {batch_group.nodes}."
            )

        response = await self.modbus_function_map[function](
            client, batch_group.start_addr, batch_group.size, self.communication_options.slave_id, False
        )
    ...
    async def read_node(self, client: ModbusRTUClient, node: ModbusRTUNode) -> float | int | bool:
        ...
        try:
            ...
            if node.options.function not in self.modbus_function_map:
                raise ModbusException(
                    f"Unknown modbus function {node.options.function} while trying to read node {node.config.name}."
                )
            ...
            node.set_connection_state(False)
            raise ModbusException(
                f"Unexpected error reading node {node.config.name} from device {self.name} with id {self.id}"
            ) from e
