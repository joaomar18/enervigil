...
from controller.node.node import Node, ModbusRTUNode, OPCUANode, MQTTNode
...
from model.controller.protocol.mqtt import MQTTOptions, MQTTNodeOptions, MQTT_TO_INTERNAL_TYPE_MAP
...
from controller.meter.protocol.mqtt.mqtt_device import MQTTEnergyMeter
...
from web.parsers.protocol.mqtt import parse_mqtt_meter_comm_options, parse_mqtt_node_protocol_options
...
# MQTT Protocol
def _mqtt_meter_record_factory(
    id: int,
    name: str,
    protocol: str,
    type: str,
    ...
    """Create an EnergyMeterRecord instance for an MQTT device."""
    ...
    return EnergyMeterRecord(
        ...
        type=EnergyMeterType(type),
        options=EnergyMeterOptions.cast_from_dict(options_dict),
        communication_options=MQTTOptions.cast_from_dict(communication_options_dict),
        nodes=nodes,
        id=int(id),
    )


def _mqtt_node_record_factory(
    name: str,
    protocol: str,
    config_dict: Dict[str, Any],
    protocol_options_dict: Dict[str, Any],
    ...
    """Create a NodeRecord instance for an MQTT node."""

    return NodeRecord(
        name=str(name),
        protocol=Protocol(protocol),
        config=BaseNodeRecordConfig.cast_from_dict(config_dict),
        protocol_options=MQTTNodeOptions.cast_from_dict(protocol_options_dict),
        attributes=NodeAttributes.cast_from_dict(attributes_dict),
    )


def _mqtt_node_factory(record: NodeRecord) -> Node:
    """Creates an MQTTNode instance with topic/mode configuration."""

    if not isinstance(record.protocol_options, MQTTNodeOptions):
        raise TypeError(f"Expected MQTTNodeOptions for protocol MQTT, got {type(record.protocol_options).__name__}")
    internal_type = MQTT_TO_INTERNAL_TYPE_MAP[record.protocol_options.type]
    config = NodeConfig.create_config_from_record(record, internal_type)
    return MQTTNode(configuration=config, protocol_options=record.protocol_options)
...
ProtocolRegistry.register_protocol(
    ...
    MQTTOptions,
    MQTTNodeOptions,
    _mqtt_meter_record_factory,
    _mqtt_node_record_factory,
    _mqtt_node_factory,
    parse_mqtt_meter_comm_options,
    parse_mqtt_node_protocol_options,
)
