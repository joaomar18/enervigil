...
from conf.env import TIMEDB_HOSTNAME, TIMEDB_PORT, TIMEDB_USERNAME, TIMEDB_PASSWORD
...
class TimeDBClient:
    ...
    async def init_connection(self) -> None:
        ...
        if TIMEDB_HOSTNAME is None or TIMEDB_PORT is None:
            raise ValueError(f"Time DB hostname or port were not specified in the environment.")
               
        self.client = InfluxDBClient(host=TIMEDB_HOSTNAME, port=TIMEDB_PORT, username=TIMEDB_USERNAME or "root", password=TIMEDB_PASSWORD or "root")
    ...
    def __get_new_client(self) -> InfluxDBClient:
        ...
        return InfluxDBClient(host=TIMEDB_HOSTNAME, port=TIMEDB_PORT, username=TIMEDB_USERNAME or "root", password=TIMEDB_PASSWORD or "root")
